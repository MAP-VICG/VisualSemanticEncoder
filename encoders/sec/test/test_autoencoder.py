...
class EncoderTests(unittest.TestCase):
    ...
    def test_simple_vae(self):
        """
        Tests if autoencoder model is build with the correct size fo input, output and encoding
        """
        factory = ModelFactory(2048, 128, 2048)
        model = factory.simple_vae()

        self.assertEqual([(None, 2048)], model.encoder.layers[0].input_shape)
        self.assertEqual((None, 128), model.encoder.layers[-1].output_shape)
        self.assertEqual((None, 2048), model.decoder.layers[-1].output_shape)
    ...
    def test_factory_call_vae(self):
        """
        Tests if the correct type of AE is returned in call method
        """
        model = ModelFactory(2048, 128, 2048)(ModelType.SIMPLE_VAE)

        self.assertEqual([(None, 2048)], model.encoder.get_layer('ae_input').input_shape)
        self.assertEqual((None, 128), model.encoder.get_layer('code').output_shape)
        self.assertEqual((None, 2048), model.decoder.get_layer('ae_output').output_shape)

    def test_fit_ae(self):
        """
        ...
        expected_keys = ['best_loss', 'loss', 'val_loss']

        self.assertEqual(3, len(ae.history['loss']))
        self.assertEqual(3, len(ae.history['val_loss']))
        self.assertTrue(isinstance(ae.best_weights, list))
    ...
    def test_fit_vae(self):
        ...
        data = loadmat('../../../../Datasets/SAE/cub_demo_data.mat')
        input_length = output_length = data['X_tr'].shape[1] + data['S_tr'].shape[1]
        ae = Encoder(input_length, data['S_tr'].shape[1], output_length, ModelType.SIMPLE_VAE, 3)
        ae._fit(data['X_tr'], data['S_tr'])
        expected_keys = ['best_loss', 'loss', 'kl_loss', 'reconstruction_loss']

        self.assertEqual(3, len(ae.history['loss']))
        self.assertEqual(3, len(ae.history['kl_loss']))
        self.assertEqual(3, len(ae.history['reconstruction_loss']))
        self.assertTrue(isinstance(ae.best_weights, list))
        self.assertEqual(expected_keys, list(ae.history.keys()))
    ...
    def test_estimate_semantic_data_awa_vae(self):
        """
        Tests if result from semantic data estimation is in the correct shape for awa
        """
        data = loadmat('../../../../Datasets/SAE/awa_demo_data.mat')
        input_length = output_length = data['X_tr'].shape[1] + data['S_tr'].shape[1]
        ae = Encoder(input_length, data['S_tr'].shape[1], output_length, ModelType.SIMPLE_VAE, 1)
    ...
    def test_estimate_semantic_data_cub_vae(self):
        """
        Tests if result from semantic data estimation is in the correct shape for cub
        """
        data = loadmat('../../../../Datasets/SAE/cub_demo_data.mat')
