"""
Contains demo for running SAE (Semantic Auto-encoder) for SUN Attributes dataset. This approach was proposed by
...
class SUN:
    def __init__(self, data_path):
        ...
        :param data_path: string with path with .mat file with dataset
        """
        self.hit_k = 3
        self.lambda_ = 50000
        self.z_score = True
        ...
        random.seed(1519)
        test_labels = list(np.array(random.sample(range(np.max(self.data['img_class'])), 10)) + 1)
        tr_mask = [False if img in test_labels else True for img in self.data['img_class'][0]]
        te_mask = [True if img in test_labels else False for img in self.data['img_class'][0]]
    ...
    def set_semantic_data(self, sem_data=None):
        ...
        :param sem_data: array of shape (14140, 102) when seed is 1519
        :return: None
        """
        if self.s_tr is None and sem_data is None:
            self.s_tr = self.data['S_tr']
        elif sem_data is not None:
            if sem_data.shape == self.data['S_tr'].shape:
                self.s_tr = sem_data
            else:
                raise ValueError('Data provided is invalid. It should be of shape %s' % str(self.data['S_tr']))
...
if __name__ == '__main__':
    sun = SUN('../../../../Datasets/sun_data_resnet50.mat')
    sun.set_semantic_data()
    print('\n[1] SUN ZSL accuracy [V >>> S]: %.1f%%\n' % (sun.v2s_projection() * 100))
    print('[2] SUN ZSL accuracy [S >>> V]: %.1f%%\n' % (sun.s2v_projection() * 100))
