...
from _old.src.utils import Normalization
...
class NormalizationTests(unittest.TestCase):
    ...
    def test_norm_zero_one(self):
        ...
        self.assertEqual(1, round(sem_fts.max()))
        self.assertEqual(0, round(sem_fts.min()))
    ...
    def test_norm_visual(self):
        ...
        for col in range(sem_fts.shape[1]):
            self.assertEqual(1, round(sem_fts[:, col].max()))
            self.assertEqual(0, round(sem_fts[:, col].min()))
