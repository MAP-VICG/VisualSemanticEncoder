...
def cal_acc(te_est_or):
    ...
    acc_value, _ = ZSL.zsl_el(te_est_st, prototypes, test_labels_st, template_labels, 3, False)
    return acc_value


def cal_acc_sec(te_est):
    acc_value, _ = ZSL.zsl_el(te_est, _data['S_te_pro'], test_labels, template_labels, 3, False)
    return acc_value


_data = loadmat('../Datasets/apy_data_resnet50.mat')
tr_mask = [False if img.startswith('Yahoo') else True for img in _data['img_list']]
te_mask = [True if img.startswith('Yahoo') else False for img in _data['img_list']]
